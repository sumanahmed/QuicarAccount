            ...
            <div class="col-12">
                <div class="card">
                    <div class="car-header">
                      <form class="form" action="{{ route('car_year.index') }}" method="get" style="padding:10px 20px;">
                        <div class="row">
                          <div class="col-md-4">
                            <div class="form-group">
                              <label for="name">Year</label>
                              <input type="text" name="name" class="form-control" @if(isset($_GET['name'])) value="{{ $_GET['name'] }}" @endif placeholder="Enter year" />
                            </div>
                          </div>
                          <div class="col-md-4">
                            <div class="form-group">
                          ...
                          @foreach($years as $year)
                            <tr class="year-{{ $year->id }}">
                              ...
                              <td style="vertical-align: middle;text-align: center;">
                                  <button class="btn btn-xs btn-warning" data-toggle="modal" id="edit" data-target="#editModal" data-id="{{ $year->id }}" data-name="{{ $year->name }}" title="Edit">Edit</button>
                                  <button class="btn btn-xs btn-danger" data-toggle="modal" id="delete" data-target="#deleteModal" data-id="{{ $year->id }}" title="Delete">Delete</button>
                              </td>
                            </tr>
                          @endforeach
                        </tbody>
...
@endsection
@section('scripts')
    <script src="{{ asset('assets/js/car-year.js') }}"></script>
    <script>
        $('.nav-car-year').addClass('active');
    </script>
@endsection